...
public class PostgreSqlChecker : IChecker
{
  public bool Supports(string protocol) => protocol.Equals("postgresql", StringComparison.OrdinalIgnoreCase) ||
                                            protocol.Equals("postgres", StringComparison.OrdinalIgnoreCase);
  ...
  public async Task<CheckResult> CheckAsync(ServiceConfig service, string path)
  {
    ...
    try
    {
      var database = service.Database ?? "postgres";
      var username = service.Username ?? "postgres";
      var password = service.Password ?? "";
      var connectionString = $"Host={service.Host};Port={service.Port};Database={database};Username={username};Password={password};Timeout={(int)(service.Timeout?.TotalSeconds ?? 30)}";
      ...
      {
        await using var command = new NpgsqlCommand(service.Query, connection);
